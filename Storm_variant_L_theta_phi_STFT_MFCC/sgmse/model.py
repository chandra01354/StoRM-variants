...
from sgmse.loss_stft import multi_resolution_stft_loss
...
###########################################################################################################################################
                ...
                #sc_loss, mag_loss, mfcc_loss = compute_mean_stft_loss(y_denoised, x)


                # Assuming `multi_resolution_stft_loss` and `mfcc_loss` are your loss functions
                sc_loss, mag_loss, mfcc_loss = multi_resolution_stft_loss(y_denoised, x)
                # Ensure all losses are on the same device as one of the input tensors (e.g., `x` or `y_denoised`)
                device = x.device  # Use the device of the `x` tensor or `y_denoised`

                # Move all losses to the same device
                loss_denoiser = loss_denoiser.to(device)
                loss_score = loss_score.to(device)
                sc_loss = sc_loss.to(device)
                mag_loss = mag_loss.to(device)
                mfcc_loss = mfcc_loss.to(device)
                '''
                print("loss_denoiser.shape", loss_denoiser.shape)
                print("loss_score.shape", loss_score.shape)
                print("sc_loss.shape", sc_loss.shape)
                print("mag_loss.shape", mag_loss.shape)
                print("mfcc_loss.shape", mfcc_loss.shape)
                '''
                # Now calculate the total loss
                loss = (loss_denoiser + loss_score) + (sc_loss) + (mag_loss) + (mfcc_loss)
