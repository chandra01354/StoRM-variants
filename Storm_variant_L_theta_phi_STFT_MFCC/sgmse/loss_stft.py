...
def multi_resolution_stft_loss(x, y, sample_rate=16000,fft_sizes=[1024, 2048, 512], hop_sizes=[120, 240, 50], win_lengths=[600, 1200, 240], window="hann_window", factor_sc=0.1, factor_mag=0.1, factor_mfcc=0.1):
    ...
    for i in range(x.size(0)):
        ...
        mf_l = MFCCLoss(x[i], y[i], sample_rate=16000, n_fft=510, hop_length=256, win_length=510)
        total_mfcc_loss.append(mf_l.mean()) 
        #print("x[i].shape", x[i].shape)
        #print("x_waveform.shape", x_waveform.shape)
    ...
    mean_mfcc_loss = normalized_mfcc_loss.mean()
        ...
        #mfcc_loss+=mf_l
    #mfcc_loss /= x.size(0)
    mfcc_loss = mean_mfcc_loss
    return factor_sc * sc_loss, factor_mag * mag_loss, factor_mfcc * mfcc_loss
