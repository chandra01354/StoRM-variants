...
########################################################################################################################
        '''
        alpha = 2.0         # Controls the strength of skewness
        # Generate uniform samples in the range (t_eps, sde_T)
        t = torch.rand(x.shape[0], device=x.device) * (self.sde.T - self.t_eps) + self.t_eps
        ...
        # Transform to skewed distribution, more density towards t_eps
        skewed_samples = t_normalized ** alpha

        # Rescale back to the original range (t_eps, sde_T)
        t = skewed_samples * (self.sde.T - self.t_eps) + self.t_eps
        '''
        '''
        lambda_param = 5.0  # Controls the skewness of the exponential distribution
        # Generate uniform samples in the range (t_eps, sde_T)
        ...
        # Transform to exponential distribution skewed towards 0
        exponential_samples = torch.exp(-lambda_param * t_normalized)
